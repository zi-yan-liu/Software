...
void encoderRcallback(const std_msgs::Int64::ConstPtr& msg){
  ...
  Pose2d_Pose2d_Factor* constraint = new Pose2d_Pose2d_Factor(pose_nodes[last_odom_index], pose_nodes[current_index], odometry, odom_noise3);
...
void apriltagscallback(const apriltags2_ros::AprilTagDetectionArray::ConstPtr& msg){
  ...
  if (size > 0){
    ...
    if (find (landmarks_id.begin(), landmarks_id.end(), detection.id[0]) == landmarks_id.end()){
      ...
      Pose2d_Point2d_Factor* constraint = new Pose2d_Point2d_Factor(pose_nodes[last_odom_index], new_point_node, measurement, noise2);
      slam_ptr->add_factor(constraint);
      ...
      //Point2d measurement(point.getX(), point.getY());
      //cout << "The tag " << detection.id[0] << "at" << point.getX() << point.getY() << point.getZ();
      Pose2d_Point2d_Factor* constraint = new Pose2d_Point2d_Factor(pose_nodes[last_odom_index], point_nodes[index], measurement, noise2);
    ...
    for(int it=0; it< tag_detection_array.detections.end()-tag_detection_array.detections.begin(); it++){
      if(tag_detection_array.detections[it].id[0] != 424)
        ...
        if (find (watchtower_id.begin(), watchtower_id.end(), wt_id) == watchtower_id.end()){
          ...
          Point2d_Node* new_point_node = new Point2d_Node();
          watchtower_point_node.push_back(new_point_node);
          ...
          //Point2d measurement(-detection.pose.pose.pose.position.x, -detection.pose.pose.pose.position.y);
          Point2d measurement(detection.pose.pose.pose.position.x, detection.pose.pose.pose.position.y);
          Pose2d_Point2d_Factor* constraint = new Pose2d_Point2d_Factor(pose_nodes[last_odom_index], watchtower_point_node[index], measurement, noise2);
...
void gounrd_truth_callback(const duckietown_msgs::GlobalPoseArray::ConstPtr& msg){
  ...
  if(origin_set != true){
    Pose2d_Node* new_pose_node = new Pose2d_Node();
    slam_ptr->add_node(new_pose_node);
    Pose2d origin(pose_x, pose_y, pose_yaw);
    pose_nodes.push_back(new_pose_node);
...
void process(){
  ...
  for (it=0; it< watchtower_point_node.end()-watchtower_point_node.begin(); it++)
  {

    watchtower_marker_ptr->id = it;
    watchtower_marker_ptr->action = visualization_msgs::Marker::ADD;
    watchtower_marker_ptr->pose.position.x = watchtower_point_node[it]->value().x();
    watchtower_marker_ptr->pose.position.y = watchtower_point_node[it]->value().y();
    watchtower_marker_ptr->pose.position.z = 0;
...
int main(int argc, char **argv){
  ...
  //ros::Subscriber watchtower12 = n.subscribe("/watchtower12/tag_detections", 10, std::bind(&watchtowercallback, _1, 12));
  /*
  ros::Subscriber watchtower13 = n.subscribe("/watchtower13/tag_detections", 10, boost::bind(watchtowercallback, _1, 13));
  ros::Subscriber watchtower25 = n.subscribe("/watchtower25/tag_detections", 10, boost::bind(watchtowercallback, _1, 25));
  ros::Subscriber watchtower26 = n.subscribe("/watchtower26/tag_detections", 10, boost::bind(watchtowercallback, _1, 26));
  ros::Subscriber watchtower29 = n.subscribe("/watchtower29/tag_detections", 10, boost::bind(watchtowercallback, _1, 29));
  ros::Subscriber watchtower32 = n.subscribe("/watchtower32/tag_detections", 10, boost::bind(watchtowercallback, _1, 32));
  ros::Subscriber watchtower33 = n.subscribe("/watchtower33/tag_detections", 10, boost::bind(watchtowercallback, _1, 33));
  ros::Subscriber watchtower35 = n.subscribe("/watchtower35/tag_detections", 10, boost::bind(watchtowercallback, _1, 35));
  ros::Subscriber watchtower36 = n.subscribe("/watchtower36/tag_detections", 10, boost::bind(watchtowercallback, _1, 36));
  ros::Subscriber watchtower37 = n.subscribe("/watchtower37/tag_detections", 10, boost::bind(watchtowercallback, _1, 37));
  ros::Subscriber watchtower38 = n.subscribe("/watchtower38/tag_detections", 10, boost::bind(watchtowercallback, _1, 38));
  ros::Subscriber watchtower40 = n.subscribe("/watchtower40/tag_detections", 10, boost::bind(watchtowercallback, _1, 40));
  ros::Subscriber watchtower42 = n.subscribe("/watchtower42/tag_detections", 10, boost::bind(watchtowercallback, _1, 42));
  ros::Subscriber watchtower43 = n.subscribe("/watchtower43/tag_detections", 10, boost::bind(watchtowercallback, _1, 43));
  ros::Subscriber watchtower47 = n.subscribe("/watchtower47/tag_detections", 10, boost::bind(watchtowercallback, _1, 47));
  */
